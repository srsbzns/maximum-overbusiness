...
resource "aws_lb_target_group" "sywl-lb-tg" {
  name = "sywl-lb-tg"
  port = "80"
  protocol = "HTTP"
  vpc_id = "vpc-10b7c774"
...
resource "aws_lb_listener" "sywl-lb-listener" {
  load_balancer_arn = "${aws_lb.sywl-lb.arn}"
  port = "80"
  protocol = "HTTP"
  default_action {
    type = "forward"
    target_group_arn = "${aws_lb_target_group.sywl-lb-tg.arn}"
  }
}

resource "aws_launch_template" "sywl-launchtemp" {
  name = "sywl-launchtemp"
...
resource "aws_autoscaling_group" "sywl-autoscalegrp" {
  availability_zones = ["us-east-1a","us-east-1b"]
  desired_capacity = 2
  max_size = 2
  min_size = 2
  health_check_type = "EC2"
  target_group_arns = ["${aws_lb_target_group.sywl-lb-tg.arn}"]
  launch_template = {
    id = "${aws_launch_template.sywl-launchtemp.id}"
    version = "$$Latest"
  }
}
